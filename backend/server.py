...
async def create_coinpal_payment(order_id: str):
    """Créer un paiement CoinPal pour une commande"""
    order = await db.orders.find_one({"id": order_id}, {"_id": 0})
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    ...
    if payment_result.get('success'):
        # Mettre à jour la commande avec les infos CoinPal
        await db.orders.update_one(
            {"id": order_id},
            {"$set": {
                "coinpal_payment_id": payment_result.get('payment_id'),
...
@api_router.get("/coinpal/check-payment/{payment_id}")
async def check_coinpal_payment(payment_id: str):
    """Vérifier le statut d'un paiement CoinPal"""
    return await coinpal_service.check_payment_status(payment_id)

@api_router.post("/coinpal/webhook")
async def coinpal_webhook(request: Request):
    """Webhook pour recevoir les notifications de CoinPal"""
    try:
...
# Import payment, oauth and admin routes
